    ...
    >
      <div className="pointer-events-none absolute inset-0">
        ...
        <div
          ...
          style={{
            backgroundImage: "radial-gradient(#B88A3B 1.2px, transparent 1.2px)",
      ...
      <div className="relative mx-auto max-w-7xl px-5 sm:px-6">
        <div className="mx-auto max-w-3xl text-center">
          <div
            className={`inline-flex items-center gap-2 rounded-full border border-[#D4AA6A]/40 bg-white/80 px-5 py-2 shadow-sm backdrop-blur-sm transition-all duration-700 ease-[cubic-bezier(0.16,1,0.3,1)] ${
          ...
          <h2
            className={`mt-5 font-serif text-3xl font-bold leading-tight text-[#1A1A1A] sm:text-4xl md:text-5xl lg:text-6xl transition-all delay-100 duration-[900ms] ease-[cubic-bezier(0.16,1,0.3,1)] ${
              visible ? "translate-x-0 opacity-100" : "-translate-x-16 opacity-0"
            }`}
          >
            Explore Our <span className="relative inline-block text-[#B88A3B]">Patches</span>
          </h2>

          <div
            className={`mt-5 flex items-center justify-center gap-3 transition-all delay-200 duration-700 ${
          ...
          <p
            className={`mx-auto mt-5 max-w-2xl text-sm leading-7 text-gray-500 sm:text-base lg:text-lg lg:leading-8 transition-all delay-300 duration-[900ms] ease-[cubic-bezier(0.16,1,0.3,1)] ${
              visible ? "translate-x-0 opacity-100" : "translate-x-16 opacity-0"
            }`}
          >
        ...
        {/* 1 col mobile, 2 cols tablet, 3 cols large screen — always 3 per row on lg */}
        <div className="mt-12 grid grid-cols-1 gap-6 sm:mt-14 sm:grid-cols-2 lg:mt-16 lg:grid-cols-3 lg:gap-7">
          {items.map((item, index) => {
            ...
            const hiddenAnimation = hiddenAnimations[index % hiddenAnimations.length];

            return (
              <Link
                key={item.slug}
                href={`/patches/${item.slug}`}
                className={`group relative overflow-hidden rounded-[28px] bg-white shadow-[0_8px_35px_rgba(0,0,0,0.06)] transition-all duration-[900ms] ease-[cubic-bezier(0.16,1,0.3,1)] hover:-translate-y-2 hover:shadow-[0_22px_55px_rgba(0,0,0,0.14)] ${
                  visible
                    ? "translate-x-0 translate-y-0 scale-100 opacity-100"
                    : `${hiddenAnimation} scale-[0.96] opacity-0`
                }`}
                style={{ transitionDelay: visible ? `${400 + index * 130}ms` : "0ms" }}
              >
